  ...
  <div class="full-page-container">
    ...
    <header class="app-header">
      <div class="header-left">
        <span class="logo">
          <img
            src="https://f/b94a41833dbdc8f942daacfef75877d2"
            alt="Logo"
            class="logo-img"
          />
        </span>
        ...
        <div class="rich-text-editor">
          <div class="toolbar">
            ...
            <div class="toolbar-group">
              ...
              <button @click="format('outdent')" title="Decrease Indent">
                ⇤
              </button>
              <button @click="format('indent')" title="Increase Indent">
                ⇥
              </button>
            </div>
            <div class="toolbar-separator"></div>
            ...
            <div class="toolbar-group">
              ...
              <button
                @click="format('formatBlock', '<pre>')"
                :class="{ active: isFormatActive('pre') }"
                title="Code"
              >
                &lt;/&gt;
              </button>
            ...
            <div class="toolbar-separator"></div>
            <div class="toolbar-group">
              <button @click="clearAllFormatting" title="Clear Formatting">
                ✖
              </button>
            </div>
          ...
          <div class="status-bar">
            ...
            <span v-if="isUploadingImage" class="upload-status"
        ...
        <div class="form-grid">
          ...
          <div class="row">
            ...
            <div class="control">
              ...
              <div class="small">
                输入自定义标签后按回车完成输入，最多可添加10个用户自定义标签
              </div>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="control">
              <div class="tags">
                <div
                  class="tag"
                  v-for="(expert, i) in selectedExperts"
                  :key="i"
                >
                  {{ expert }}
                  <span class="remove" @click="removeExpert(i)">×</span>
                </div>
              </div>
              <div
                v-if="availableExperts.length"
                class="available-experts-list"
              >
                <div
          ...
          <div class="row">
            ...
            <div class="control inline">
              <div
                class="toggle"
                @click="hasPatent = !hasPatent"
                :class="{ active: hasPatent }"
              >
                Has Patent {{ hasPatent ? '✅' : '❌' }}
              </div>
              <div
                class="toggle"
                @click="hasDemo = !hasDemo"
                style="margin-left: 10px"
                :class="{ active: hasDemo }"
              >
                Has Demo {{ hasDemo ? '✅' : '❌' }}
              </div>
            </div>
  ...
  export default {
    data() {
      return {
        ...
        // Dynamic fields from image/discussion
        ...
        imageBaseUrl: 'xx' // Your base URL prefix
      }
    },
    mounted() {
      this.$nextTick(() => {
    ...
    computed: {
      resizeHandleStyle() {
        ...
        return {
          top: `${
            imageRect.top - editorRect.top + this.$refs.editor.scrollTop
          }px`,
    ...
    methods: {
      ...
      async uploadImageToApi(file) {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Upload failed with status: ${response.status}. Response: ${errorBody}`
            )
          }
          const data = await response.json()
          if (!data.url) {
            throw new Error(
              'API response for image upload is missing the "url" property.'
            )
          }
      ...
      async insertImage() {
        ...
        input.onchange = async (e) => {
          ...
          try {
            ...
            this.insertHTML(
              `<img src="${imageUrl}" class="uploaded-image" alt="${file.name}" />`
            )
      ...
      clearAllFormatting() {
        if (
          confirm('Are you sure you want to clear all content and formatting?')
        ) {
      ...
      sanitizeHTML(html) {
        ...
        const body = doc.body
        const allowedTags = ['P','B','I','U','S','STRIKE','A','UL','OL','LI','BLOCKQUOTE','PRE','BR','DIV','SPAN','H1','H2','H3','H4','H5','H6']
        ...
        walkTheDOM(body, (currentNode) => {
          ...
          if (currentNode.nodeType === 1) {
            if (
              ['SCRIPT', 'STYLE', 'LINK', 'META', 'IFRAME', 'OBJECT'].includes(
                currentNode.tagName
              )
            ) {
              currentNode.parentNode?.removeChild(currentNode)
      ...
      format(cmd, value = null) {
        const blockCommands = ['justifyLeft','justifyCenter','justifyRight','justifyFull','insertUnorderedList','insertOrderedList','indent','outdent','formatBlock']
      ...
      applyLink() {
        if (this.currentLinkUrl) {
          if (
            !this.currentLinkUrl.startsWith('http://') &&
            !this.currentLinkUrl.startsWith('https://')
          ) {
        ...
        while (node && node !== this.$refs.editor) {
          if (node.nodeType === 1) {
            ...
            if (
              ...
              this.activeFormats.bold = true
            if (el.tagName === 'I' || style.fontStyle === 'italic')
              this.activeFormats.italic = true
            if (el.tagName === 'U' || style.textDecorationLine.includes('underline'))
              this.activeFormats.underline = true
            if (
              el.tagName === 'S' ||
              el.tagName === 'STRIKE' ||
              style.textDecorationLine.includes('line-through')
            )
              this.activeFormats.strikeThrough = true
            switch (style.textAlign) {
              case 'left':
                this.activeFormats.justifyLeft = true
                break
              case 'center':
                this.activeFormats.justifyCenter = true
                break
              case 'right':
                this.activeFormats.justifyRight = true
                break
              case 'justify':
                this.activeFormats.justifyFull = true
                break
            }
            if (el.tagName === 'UL')
              this.activeFormats.insertUnorderedList = true
            if (el.tagName === 'OL')
              this.activeFormats.insertOrderedList = true
            if (el.tagName === 'BLOCKQUOTE')
              this.activeFormats.blockquote = true
      ...
      addShortcuts() {
        this.shortcutHandler = (e) => {
          if (
            (e.ctrlKey || e.metaKey) &&
            this.$refs.editor === document.activeElement
          ) {
